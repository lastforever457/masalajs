...
function Contest() {
    ...
    const handleSearchTask = (event: MouseEvent) => {
        ...
        const searchInput = document.querySelector<HTMLInputElement>("#search-task-input");
        if (searchInput) {
            const selectedTasks = tasks.filter((task) => task.fun_name.toLowerCase().includes(searchInput.value.toLowerCase() ?? ""));
            const sortedSelectedTasks = selectedTasks.sort((a, b) => a.fun_name.localeCompare(b.fun_name));
    ...
    const handleCreateContest = async () => {
        const newContestForm = document.querySelector<HTMLFormElement>("#newContestForm");
        if (newContestForm) {
    ...
    return (
        <div className="px-5 py-3">
            <div className="contests-header d-flex justify-content-between items-center">
                <h2>Contest</h2>
                <Button variant="contained" data-bs-target="#createContestModal" data-bs-toggle="modal"
                        className=""><BiPlus/>Yaratish</Button>
            </div>
            <hr/>
            <div className="contests-body">
                {contests ? contests.map((contest, index) => {
                    ...
                    contest.users.map((usr) => {
                        const registeredUsers = users.find(user => user.id === usr);
                        if (registeredUsers) {
                    ...
                    return (
                        <Accordion
                            className={`contest-item alert alert-${generateTheme(contest.status)} `}
                            key={index}>
                            <AccordionSummary
                            ...
                            >
                                <div style={{width: "96%"}}
                                    ...
                                    <div
                                        className={`alert m-0 p-1 d-flex justify-content-center align-items-center alert-${generateTheme(contest.status)}`}>{generateStatus(contest.status)}</div>
                                </div>
                            </AccordionSummary>
                            <AccordionDetails>
                                <div style={{minHeight: "200px"}} className="contest-body">
                                    <div className="row">
                                        <div className="col-sm-6 col-md-4 col-lg-3">
                                            <div className="contest-passport">
                                                <table className="table table-bordered">
                                                    ...
                                                    <tr>
                                                        <td>Finish:</td>
                                                        <td>{contest.finishDate} | {contest.finishTime}</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Selected tasks</td>
                                                        <td>{contest.tasks.length}</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Registered users</td>
                                                        <td>{contest.users.length}</td>
                                                    </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
                                        {contest.status === "started" || contest.status === "finished" ? (
                                            <div className="col-sm-6 col-md-4 col-lg-3">
                                                ...
                                                <div className="users-list">
                                                    <ul className="list-group list-group-flush bg-transparent">
                                                        {registeredUsersArr.map((user) => {
                                                            return (
                                                                <li key={user.id}
                                                                    className="list-group-item">{user.name}</li>
                                                            )
                                                        })}
                                                    </ul>
                                                </div>
            ...
            <div className="modal fade modal-xl" id="createContestModal" tabIndex={-1}
                 aria-labelledby="createContestModalLabel"
                 aria-hidden="true">
                <div className="modal-dialog modal-dialog-scrollable">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h1 className="modal-title fs-5" id="createContestModalLabel">Yangi turnir e'lon qilish</h1>
                                    ...
                                    <div className="contest-form">
                                        ...
                                        <form name="newContestForm" id="newContestForm">
                                            <div className="my-3">
                                                ...
                                                <input name="contestName" type="text" className="form-control mt-2"
                                                       id="newContestName"/>
                                            </div>
                                ...
                                <div className="col-lg-6">
                                    <div className="newContestTasks">
                                        ...
                                        <div className="my-2 d-flex justify-content-start gap-3 align-items-center">
                                            <label htmlFor="search-task-input">Qidirish:</label>
                                            {/* eslint-disable-next-line @typescript-eslint/ban-ts-comment*/}
                                            {/* @ts-ignore*/}
                                            <input onChange={handleSearchTask} name="search-task-input" type="search"
                                                   placeholder="Search"
                                                    ...
                                                    <div className="d-flex start align-items-center">
                                                        <div
                                                            ...
                                                            <input type="checkbox" className="form-check-input me-2"
                                                                   ...
                                                                   name="contestTasks"
                                                            />
                                                            <label htmlFor={task.id.toString()}>{task.fun_name}</label>
                                                        </div>
                                                    </div>
                                                </li>
                                            ))}
                        ...
                        <div className="modal-footer">
                            <div className="input-group d-flex justify-content-end align-items-center">
                                <button data-bs-dismiss="modal" className="btn btn-secondary">Bekor
                                    qilish
                                </button>
                                <button onClick={handleCreateContest} className="btn btn-primary">Yaratish</button>
                            </div>
                        </div>
                    </div>
                </div>
